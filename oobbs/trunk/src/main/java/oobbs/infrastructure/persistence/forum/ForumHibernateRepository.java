...
//				Post latestPost = (Post) session.load(Post.class, latestPostId);
	...
	@Override
	public void process(SupportRequest request, SupportResponse response) {
		String requestName = request.getName();
		if(SupportRequest.COUNT_FORUM_POSTS.equals(requestName)){
			Forum forum = (Forum)request.getParam("forum");
			Long postsCount = getForumPostsCount(forum);
			response.setResult(postsCount);
		}else if(SupportRequest.GET_FORUM_LATEST_POST.equals(requestName)){
			Forum forum = (Forum)request.getParam("forum");
			Post lastestPost = getForumLatestPost(forum);
			response.setResult(lastestPost);
		}
	}

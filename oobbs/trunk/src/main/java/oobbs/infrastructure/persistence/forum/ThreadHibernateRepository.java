...
public class ThreadHibernateRepository extends AbstractHibernateRepository<Thread, Long> implements ThreadRepository{
	...
	@Override
	public Forum getForum(final Long threadId) {
		return getHibernateTemplate().executeWithNativeSession(new HibernateCallback<Forum>() {
			@Override
			public Forum doInHibernate(Session session)	throws HibernateException, SQLException {
				String getForumOfThreadHql = "select f from Thread t join t.forum f where t.id = :threadId";
				Forum forum = (Forum) session.createQuery(getForumOfThreadHql).setParameter("threadId", threadId).setCacheable(true).uniqueResult();
				return forum;
			}
		});
	}

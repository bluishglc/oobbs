...
import oobbs.presentation.util.Pager;
...
public class ThreadAction extends BaseAction implements NavigationPathProvider {
    ...
    public List<NavigationNode> getNavigationPath(String actionName) {
        if("navToCreateThread".equals(actionName)){
//			ForumDTO forumDTO = threadService.getForum(threadId);
//			ForumGroupDTO forumGroupDTO = forumService.getForumGroup(forumDTO.getId());
//			NavigationPathBuilder builder = new NavigationPathBuilder();
//			builder.buildForumPath(forumGroupDTO, forumDTO);
//			builder.buildTextNode("New Thread");
//			return builder.getNavigationPath();
            return threadService.getThreadCreationNavPath(threadId);
        }else {
//			ForumDTO forumDTO = threadService.getForum(threadId);
//			ForumGroupDTO forumGroupDTO = forumService.getForumGroup(forumDTO.getId());
//			NavigationPathBuilder builder = new NavigationPathBuilder();
//			builder.buildIndexNode();
//			builder.buildForumNode(name, id)
            return threadService.getThreadNavPath(threadId);
        }
    }
